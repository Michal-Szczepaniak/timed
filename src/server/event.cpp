...
#define check_attr(txt, a, empty) \
    ...
    unsigned _ctzp = (e->has_ticker() << 12) | (e->has_time() << 8) | (e->has_timezone() << 4)
                     | (e->has_recurrence() << 0);
    bool ok = false;
    switch (_ctzp) {
#define _ok(msg) \
    log_info("new event accepted, %s", msg); \
    ok = true; \
    break;
#define _not_ok(msg) \
    log_error("a new event is rejected: %s", msg); \
    break;
    case 0x0000:
    case 0x0010:
        if (e->to_be_keeped()) {
    ...
    case 0x1110:
    case 0x1111:
        _not_ok("both epoch time and broken down time are given");
    case 0x1010:
        _not_ok("both epoch time and timezone are given");

    case 0x1000:
        _ok("epoch time is given");
    case 0x1001:
    case 0x1011:
        _ok("epoch time and recurrence pattern are given");
    case 0x0100:
        _ok("broken down time is given");
    case 0x0110:
        _ok("broken down time and the timezone are given");
    case 0x0101:
        _ok("broken down time and the recurrence pattern are given");
    case 0x0111:
        _ok("broken down time, timezone and the recurrence pattern are given");
    case 0x0001:
        _ok("recurrence pattern is given");
    case 0x0011:
        _ok("timezone and the recurrence pattern are given");
#undef _ok
#undef _not_ok
    }
    ...
    if (e->has_recurrence()) {
        ...
        for (unsigned i = 0; i < e->recrs.size(); ++i)
            os << "{"
               << str_printf("wday=0%o hour=0x%x mins=0x%llx",
                             e->recrs[i].hour,
                             e->recrs[i].mday,
                             e->recrs[i].mins)
               << "}";
...
#undef REG
    for (unsigned i = 0; i <= Maemo::Timed::Number_of_Sys_Buttons; ++i)
        action_t::codec->register_name(ActionFlags::sys_button(i),
                                       QString("sys_button_%1").arg(i).toStdString());
    for (unsigned i = 1; i <= Maemo::Timed::Max_Number_of_App_Buttons; ++i)
        action_t::codec->register_name(ActionFlags::app_button(i),
                                       QString("app_button_%1").arg(i).toStdString());
...
const char *action_t::cred_key() const
{
    if (cred_key_value == NULL) {
        ...
        if (cred_modifier != NULL) {
            os << "tokens={";
            bool first = true;
            for (map<string, bool>::const_iterator it = cred_modifier->tokens.begin();
                 it != cred_modifier->tokens.end();
                 ++it) {
...
void event_t::sort_and_run_actions(uint32_t mask)
{
    ...
    for (unsigned i = 0, have_exec = false; i < r->size(); ++i) {
        ...
        bool start_now = a.cred_key() != current_key
                         || (have_exec && i_exec); // either new key or second exec
        if (start_now && i > 0)
...
void event_t::run_actions(const vector<unsigned> &acts, unsigned begin, unsigned end)
{
    ...
    try {
        ...
        log_error("action list %d%s failed: %s",
                  cookie.value(),
                  print_vector(acts, begin, end).c_str(),
                  e.message.c_str());
        pid_t process = e.pid();
        if (process != daemon) {
    ...
    for (unsigned i = begin; i < end; ++i) {
        ...
        try {
            ...
            QString ifac = string_std_to_q(
                find_action_attribute("DBUS_INTERFACE",
                                      a,
                                      (a.flags & ActionFlags::DBus_Signal) != 0));
            ...
            if (flags & EventFlags::Trigger_When_Adjusting)
                param["ADJUSTMENT"] = QString::fromStdString(
                    state->machine->transition_time_adjustment.str());
            ...
            {
                if (bus == 0) // system bus
                    c = new QDBusConnection(
                        QDBusConnection::connectToBus(QDBusConnection::SystemBus, cname));
                else {
            ...
            log_error("%u[%d]: dbus-action not executed: %s",
                      cookie.value(),
                      acts_i,
                      e.message.c_str());
            ++dbus_fail_counter;
...
struct singleton_t
{
    singleton_t() { event_t::codec_initializer(); }
    ~singleton_t() { event_t::codec_destructor(); }
} singleton;
} // namespace
